...
- 完了
  ...
  - 地形読込エラーUIと再試行導線
  - セーフモード起動（`SAFE MODE` ボタン、`?safeMode=1`）
  - 衝突判定とリスポーン
  - PauseMenu + SettingsPanel + LicensePanel
  - localStorage設定永続化（感度・画質・キー割当）
  - Modelユニットテスト整備（vitest）
- 未完了
  - 実表示E2E品質確認の深掘り（道路密度・橋梁形状・河川連続性）
  - API統合テスト
  - E2Eテスト自動化（起動/飛行/衝突/再開）
  - Vercelデプロイ整備、運用手順書
...
## 5. 次アクション
1. 実表示品質チェック観点（道路/橋梁/河川）をチェックリスト化して記録する。
2. API統合テスト方針を `docs/api_design.md` に合わせて確定し実装する。
3. PlaywrightでE2Eテストを追加する（起動/一時停止/衝突/復帰）。
4. Vercelデプロイ設定と運用手順書を追加する。
